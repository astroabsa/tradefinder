...
# --- 5. SMART MASTER LIST (F&O + SPOT INDICES) ---
...
def get_master_data_map():
    ...
    try:
        ...
        # We need to look in 'INDEX' instrument type for both NSE and BSE
        idx_df = df[df['SEM_INSTRUMENT_NAME'] == 'INDEX']
        
        # Helper to find nearest expiry for Futures
        ...
        # Process Spot Indices (Exact Name Match)
        # Nifty 50
        ...
        # Bank Nifty
        bank = idx_df[(idx_df['SEM_EXM_EXCH_ID'] == 'NSE') & (idx_df['SEM_TRADING_SYMBOL'] == 'Nifty Bank')]
        if not bank.empty: index_map['BANKNIFTY'] = str(bank.iloc[0]['SEM_SMST_SECURITY_ID'])
        
        # Sensex (BSE) - Usually "S&P BSE SENSEX"
        # We search for "SENSEX" in the symbol to be safe
        sensex = idx_df[(idx_df['SEM_EXM_EXCH_ID'] == 'BSE') & (idx_df['SEM_TRADING_SYMBOL'].str.contains('SENSEX', case=False))]
...
def fetch_futures_data(security_id, interval=60):
    try:
        ...
        res = dhan.intraday_minute_data(
            ...
            instrument_type="FUTSTK", # Futures for Scanner
            from_date=from_date,
            to_date=to_date,
            interval=interval
        )
...
def refreshable_dashboard():
    # Setup for Spot Indices
    # Note: NSE Indices -> 'IDX_I', BSE Indices -> 'BSE_IDX'
    indices_config = [
    ...
    for item in indices_config:
        ...
        try:
            to_date = datetime.now().strftime('%Y-%m-%d')
            # Fetch Spot Data (INDEX)
            res = dhan.intraday_minute_data(
                security_id=sec_id,
                exchange_segment=segment, # Specific Segment
                instrument_type="INDEX",  # Instrument Type is INDEX
                from_date=to_date, 
                to_date=to_date,
                interval=1 
            )
    ...
    # Render Dashboard (4 Columns now)
